...
class LeaveBot:
    ...
    def get_leave_status(self, leave_date):
        """Determine leave status based on date"""
        today = datetime.now()
    ...
    def create_structured_text(self, row):
        """Create enhanced structured text for leave data"""
    ...
    def create_embeddings(self, df):
        """Create enhanced embeddings with more context"""
        records = []
    ...
    async def query_pinecone(self, query):
        try:
            ...
            if results['matches']:
                ...
                # Combine contexts with status markers
                context_parts = []
                if completed:
...
Remember: Every response must be complete, accurate, and well-formatted with all relevant information."""},
            ...
            response = await openai.ChatCompletion.acreate(
                ...
                presence_penalty=0.6,  # Encourage comprehensive responses
